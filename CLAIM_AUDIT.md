...
| 5 | Live test count is the generated value in `STATUS.md`, never a hand-typed literal | `STATUS.md` (generated by `tools/update_status.py`) | `python tools/update_status.py --check` | matches `STATUS.md`; the absolute number is environment-relative (optional-dependency-gated cases differ between the pinned CI image and a full-extras dev machine) and is masked in `--check` by design | **VERIFIED (mechanism; the count is a generated value, not a fixed literal — see `TEST_COUNT_RECONCILIATION.md`)** |
| 5b | The 306→310 episode was a stale-generated-file artifact, not a flake | `STATUS.md` history | see `TEST_COUNT_RECONCILIATION.md` | a branch base predating PR #42's added tests generated a stale count; resolved by regenerating on merged `main` | **VERIFIED (historical; superseded by the generated-source rule)** |
...
| 12 | Byte-matched against the TISEAN C binary | — | TISEAN reference binary not available in sandbox | not done; binary byte-match only | **NEEDS_EXTERNAL_CHECK (low marginal value — superseded by 12a/12b below)** |
| 12a | Surrogate engine cross-checked against an **independent** implementation | `src/bsff/reference_surrogate.py` (from-scratch numpy AAFT/IAAFT, shares no code) + `tests/test_tisean_reference.py` | `python tools/validate_tisean_reference.py` | `compare_against_reference` agrees on spectrum/marginal/covariance; p-value stable | **VERIFIED** — covers the shared-implementation-bug risk a TISEAN byte-match would address |
| 12b | Verdict p-value is **calibrated** (the deeper question a byte-match cannot answer) | `artifacts/analytic_uniformity_null.json` | `python tools/analytic_uniformity_null.py --check` | white null FPR≈α & KS uniformity not rejected (p=0.35); AR(1) bare FPR>α (measured IAAFT bias) → conjunction FPR≤α | **VERIFIED** — analytic-uniformity null; no external binary required |
...
pip install -e '.[dev]'
python -m pytest tests/ --collect-only -p no:cacheprovider | grep collected   # live count → STATUS.md
python -m pytest -q                                                            # full suite green (count in STATUS.md)
