...
def is_number(value: object) -> bool:
    return isinstance(value, (int, float))  # noqa: UP038 - keep runtime-compatible tuple form.


def top(items: list[dict[str, Any]], key: str) -> list[dict[str, Any]]:
    measurable = [item for item in items if is_number(item.get(key))]
...
def aggregate(baseline: Path | None = None, require_baseline: bool = False) -> dict[str, Any]:
    ...
    provenance = (
        ...
        else {"verdict": "FAIL", "skip_classification": "UNKNOWN"}
    )
    ...
    install_time = round(
        sum(float(cache.get("install_wall_time_seconds") or 0.0) for cache in caches),
        6,
    )
    ...
    if baseline_doc:
        ...
        longitudinal["install_time_delta_percent"] = pct(
            install_time - baseline_install,
            baseline_install,
        )
        longitudinal["cache_hit_ratio_delta"] = round(
            cache_ratio - baseline_doc.get("cache_hit_ratio", 0.0),
            6,
        )
    verdict = "PASS"
    ...
    job_count = sum(len(workflow.get("jobs", [])) for workflow in inventory.get("workflows", []))
    summary = {
        "schema_version": 1,
        ...
        "rss_measurement_coverage": (
            round(
                sum(1 for step in steps if step.get("max_rss_kb") is not None) / len(steps),
                6,
            )
            if steps
            else 0.0
        ),
        "io_measurement_coverage": (
            round(
                sum(1 for step in steps if step.get("io_read_bytes") is not None) / len(steps),
                6,
            )
            if steps
            else 0.0
...
def run(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--baseline",
        default=str(ROOT / "artifacts" / "ci" / "history" / "ci_observability_baseline.json"),
    )
